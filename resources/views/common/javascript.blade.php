...
<script type="text/javascript">
    ...
    jQuery(document).on('click', '.load-page', function () {
        closeSidebar();
        $(".system-container").html('<img style="height:120px !important;" class="centered_loader" src="{{ url("img/Ripple.gif") }}"/>');
    ...
    jQuery(document).on('submit', '.ajax-post', function () {
        ...
        btn.prepend('<img class="processing-submit-image centered_loader" style="height: 50px;margin:-10px !important;" src="{{ url("img/Ripple.gif") }}">');
...
//        showLoading();
    ...
    function ajaxLoad(url, s_class, active_tab) {
        if (s_class) {
            ...
            $("." + s_class).html('<img style="height:120px !important;" class="centered_loader" src="{{ url("img/Ripple.gif") }}"/>');
        }
        jQuery(".loading-img").show();
        var material_active = $('input[name="material_page_loaded"]').val();
        if (!material_active) {
...
//                return false;
    ...
    function showLoading() {
        $(".alert_status").remove();
        $('.ajax-post').not(".persistent-modal .modal-body").prepend('<div id="" class="alert alert-success alert_status"><img style="" class="loading_img" src="{{ url("img/ajax-loader.gif") }}"></div>');
    }
...
//        $("select[name='"+name+"']").html('');
    ...
    function showRequest(formData, jqForm, options) {
        ...
        btn.attr('disabled', true);
        $(".alert_status").remove();
        $('.file-form').prepend('<div id="" class="alert alert-success alert_status"><img style="" class="loading_img" src="{{ url("img/ajax-loader.gif") }}"></div>');
    }
